...
class AnkizinHelper:
    ...
    def __init__(self):
        self.DELETE_TAG_PATTERN = re.compile(r"#Ankizin_v\d+::!DELETE(::(?:update|duplicate))?")
    ...
    def setup_collection_config(self):
        config_settings = { # https://github.com/ankitects/anki/blob/main/rslib/src/config/bool.rs
            "activeCols": ['noteFld', 'deck', 'noteCrt', 'template', 'cardDue', 'noteMod'],
            "sortType": u'noteCrt',
            "sortBackwards": False,
            "noteSortType": u'noteCrt',
    ...
    def optimize_settings(self):
        ...
        try:             
            ...
            did = mw.col.decks.id("Ankizin") # only works for very default default deck idk mabye ill fix it later
            ...
            if default_conf:
                ...
                mw.col.decks.set_config_id_for_deck_dict(deckDict, default_conf['id'])
                mw.col.decks.save(default_conf)
            else:
                showInfo("Keine Deckeinstellung gefunden. Schreib uns gerne auf Discord an.")
    ...
    def run_first_time_setup(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            CollectionOp(parent=mw, 
                        op=lambda _: self.handle_first_time_options()
    ...
    def find_new_cards_from_update(self):
        ...
        target_tag = f"#Ankizin_v{latest_version}::§NEW_CARDS::v{latest_version}"
    ...
    def run_ankizin_update_setup(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            CollectionOp(parent=mw, 
                        op=lambda _: self.handle_update_options()
            ).success(AnkizinHelper.on_success).run_in_background()
