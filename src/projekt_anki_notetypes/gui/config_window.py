...
class NotetypesConfigWindow:
    ...
    def __init__(self, clayout_: CardLayout = None):
        ...
        self.clayout = None
        if clayout_:
            if clayout_.model["name"] in projekt_anki_notetype_names():
                ...
                showInfo(
                    "Wenn du den Notiztyp hier änderst, wirst du in der Vorschau keine Änderungen sehen.\n\n"
                    f'Stattdessen kannst du den Notetype "{base_name}" editieren '
                    "Dieser Notiztyp wird davon auch geändert."
                )
    ...
    def open(self):
        ...
        for notetype_name in sorted(projekt_anki_notetype_names()):
    ...
    def _import_notetype(self, notetype_name: str) -> None:
        model = projekt_anki_notetype_model(notetype_name)
        model["id"] = 0
    ...
    def _apply_setting_changes_for_all_notetypes(self):
        for notetype_name in projekt_anki_notetype_names():
            for model in _note_type_versions(notetype_name):
                if not model:
                    continue
                ntss = ntss_for_model(model)
                self._safe_update_model_settings(
...
def _note_type_versions(note_type_name: str) -> List["NotetypeDict"]:
    ...
    models = [
        ...
        if x.name == note_type_name
        or re.match(ANKIHUB_NOTETYPE_RE.format(notetype_name=note_type_name), x.name)
...
def _names_of_all_supported_note_types() -> List[str]:
    ...
    return [
        version["name"]
        for base_name in projekt_anki_notetype_names()
        for version in _note_type_versions(base_name)
    ]
